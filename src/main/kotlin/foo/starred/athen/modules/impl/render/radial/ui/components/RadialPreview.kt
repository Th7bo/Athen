        ...
        renderState {
            ...
            provider = { graphics ->
                ...
                if (d0 >= 225f) {
                    if (!b0 && s0 in list0.indices) {
                        h1 = when {
                            RadialMenu.type >= 2 -> RadialRenderState.hitRing(mouseX, mouseY, x1, y1, num0, RadialMenu.radius2, ex0.map { it.first }, false, RadialMenu.thickness)
                            RadialMenu.type == 1 -> RadialRenderState.hitNested(mouseX, mouseY, x1, y1, num0, RadialMenu.radius2, s0, list0[s0].sub.size, false, RadialMenu.thickness)
                            else -> -1
                        }
