    ...
    init {
        ...
        `category$toggle` = rectangle {
            ...
            on<MouseEvent.Press> {
                ...
                rows0[name]?.label?.color = CascadeGeometricColor(if (Keybinds.categories.value.find { it.name == name }?.enabled != true) MochaColorScheme.Overlay0.argb else MochaColorScheme.Lavender.argb)
                buttons()
                cancel()
