...
class ActionCategoryBar(
    ...
    fun draw(graphics: GuiGraphics, mx: Int, my: Int, sx: Int, sy: Int, sh: Int, modalOpen: Boolean, zones: MutableList<UIZone>) {
        graphics.rectangle(sx, sy, sidebarW, sh, Mocha.Base.argb)
        graphics.outline(sx, sy, sidebarW, sh, 1, Mocha.Surface0.argb)
        ...
        if (selected == null) graphics.rectangle(lx, cy, lw, rowH, Mocha.Surface0.argb)
        else if (!modalOpen && mx in lx until lx + lw && my in cy until cy + rowH) graphics.rectangle(lx, cy, lw, rowH, Mocha.Surface0.withAlpha(0.5f))
        graphics.extractText("All", lx + 4, cy + (rowH - client.font.lineHeight) / 2 + 1, false, if (selected == null) Mocha.Mauve.argb else Mocha.Subtext0.argb)
        ...
        for ((i, cat) in cats.withIndex()) {
            ...
            if (deleting == cat.name) {
                graphics.rectangle(lx + 1, cy + 1, lw - 2, rowH - 2, Mocha.Red.withAlpha(0.15f))
                graphics.outline(lx + 1, cy + 1, lw - 2, rowH - 2, 1, Mocha.Red.argb)
                graphics.enableScissor(lx + 4, cy, lx + lw - 18, cy + rowH)
                graphics.extractText(cat.name, lx + 4, cy + (rowH - client.font.lineHeight) / 2 + 1, false, Mocha.Red.argb)
                ...
                if (selected == cat.name) graphics.rectangle(lx, cy, lw, rowH, Mocha.Surface0.argb)
                else if (b) graphics.rectangle(lx, cy, lw, rowH, Mocha.Surface0.withAlpha(0.5f))
                ...
                graphics.enableScissor(lx + 4, cy, lx + lw - 18, cy + rowH)
                graphics.extractText(cat.name, lx + 4, cy + (rowH - client.font.lineHeight) / 2 + 1, false, when {
